...
export function generateSvg(data: DataExport, cfg: ShapegridConfig): string {
  ...
  const cellHeight = (scaled: number): number =>
    Math.max(0.008, scaled * heightScale * 0.12 + 0.008);
  ...
  const projBase = (wx: number, wy: number, wz: number): [number, number] =>
    toScreen(wx, wy, wz);
  ...
  function squareCorners(cell: CellRenderData, wy: number): Array<[number, number]> {
  ...
  function hexProjected(cell: CellRenderData, wy: number): Array<[number, number]> {
    const wx = cell.cx - 0.5;
    const wz = cell.cy - 0.5;
    const pts: Array<[number, number]> = new Array(6);
    for (let i = 0; i < 6; i++) {
      pts[i] = projBase(
        wx + HEX_CORNER_DIRS[i][0] * halfSize,
        wy,
        wz + HEX_CORNER_DIRS[i][1] * halfSize
      );
    }
    return pts;
  }
  ...
  let trim: { x0: number; y0: number; x1: number; y1: number } = { x0: 0, y0: 0, x1: W, y1: H };
  if (cells.length > 0) {
    ...
    for (const wx of [gMinX, gMaxX]) {
      for (const wy of [0, gMaxH]) {
        for (const wz of [gMinZ, gMaxZ]) corners.push(toScreen(wx, wy, wz));
      }
    }
