...
function widgetOrigin(ctx: WidgetRenderCtx, wPx: number, hPx: number): { x: number; y: number } | null {
  ...
  let y: number;
  if (ctx.w.position === 'left' || ctx.w.position === 'right') y = (anchor.y / 100) * ctx.H - hPx / 2;
  else if (ctx.w.position === 'bottomLeft' || ctx.w.position === 'bottomRight') y = (anchor.y / 100) * ctx.H - hPx;
  else y = (anchor.y / 100) * ctx.H;
  return { x, y };
}
