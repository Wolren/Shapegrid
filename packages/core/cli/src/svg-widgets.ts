...
const ZONE_ANCHORS: Record<string, { x: number; y: number; align: 'start' | 'end' | 'center' }> = {
...
function widgetSetting(w: DashboardWidgetConfig, key: string, fallback: any): any {
  const s = w.settings ?? {};
...
function widgetOrigin(ctx: WidgetRenderCtx, wPx: number, hPx: number): { x: number; y: number } | null {
  ...
  let x: number;
  if (anchor.align === 'end') x = (anchor.x / 100) * ctx.W - wPx;
  else if (anchor.align === 'center') x = (anchor.x / 100) * ctx.W - wPx / 2;
  else x = (anchor.x / 100) * ctx.W;
  const contH = ctx.H - WIDGET_CONTAINER_TOP_INSET;
  let y: number;
  if (ctx.w.position === 'left' || ctx.w.position === 'right') y = WIDGET_CONTAINER_TOP_INSET + (anchor.y / 100) * contH - hPx / 2;
  else if (ctx.w.position === 'bottomLeft' || ctx.w.position === 'bottomRight') y = WIDGET_CONTAINER_TOP_INSET + (anchor.y / 100) * contH - hPx;
  else y = WIDGET_CONTAINER_TOP_INSET + (anchor.y / 100) * contH;
  return { x, y };
}
...
function renderLegend(ctx: WidgetRenderCtx, data: DataExport): string {
  const accent = ctx.accent;
  const secondary = ctx.secondary;
  const scale = ctx.scale;
  ...
  const stops = ramp.map((c, i) =>
    `<stop offset="${((i / (ramp.length - 1)) * 100).toFixed(0)}%" stop-color="${c}"/>`
  ).join('');
...
function renderStats(ctx: WidgetRenderCtx, data: DataExport): string {
  const accent = ctx.accent;
  const secondary = ctx.secondary;
  const scale = ctx.scale;
  const ramp = accentRamp(accent, 5);
  ...
  const big = (val: string, label: string, x: number) => `
...
function renderTimeline(ctx: WidgetRenderCtx, data: DataExport): string {
  const accent = ctx.accent;
  const scale = ctx.scale;
  const maxDays = (widgetSetting(ctx.w, 'days', 90) as number) || 90;
  ...
  const xScale = (i: number) => (count > 1 ? (i / (count - 1)) * (svgW - 2) + 1 : svgW / 2);
  ...
  const top3 = recentDays
    ...
    .map(x => x.idx);
  const peakDots = top3.map(idx =>
    `<circle cx="${xScale(idx).toFixed(1)}" cy="${yScale(recentDays[idx].contributionCount).toFixed(1)}" r="2" fill="${accent}" stroke="#0d1117" stroke-width="1"/>`
  ).join('');
...
function renderLanguages(ctx: WidgetRenderCtx, data: DataExport): string {
  const secondary = ctx.secondary;
  const scale = ctx.scale;
  const maxItems = (widgetSetting(ctx.w, 'maxItems', 5) as number) || 5;
...
function renderStreak(ctx: WidgetRenderCtx, data: DataExport): string {
  const accent = ctx.accent;
  const secondary = ctx.secondary;
  const scale = ctx.scale;
  const days = data.days ?? [];
  ...
  if (days.length === 0) {
    ...
    const col = (val: string, label: string, x: number) => `
...
const RENDERERS: Record<string, (ctx: WidgetRenderCtx, data: DataExport) => string> = {
  legend: renderLegend,
  stats: renderStats,
...
export function renderDashboardWidgets(data: DataExport, widgets: DashboardWidgetConfig[] | undefined, W: number, H: number): string {
  ...
  for (const w of visible) {
    ...
    const ctx: WidgetRenderCtx = { w, accent, secondary, scale, opacity, W, H };
    try {
