 ...
 */
export async function fetchContributions(username, from, to, token) {
    ...
    const results = await Promise.all(chunks.map(({ from, to }) => fetchContributionsSingle(username, from, to, token)));
 ...
 */
async function fetchContributionsSingle(username, from, to, token) {
 ...
 */
export function mapContributionsToCells(contributions, cellCount, dateRange) {
    ...
    const filtered = contributions.days.filter(d => d.date >= startStr && d.date <= endStr);
...
export function intensityToColor(intensity, scale = 'github') {
    ...
    switch (scale) {
        case 'github':
            ...
            return '#39d353';
        case 'warm':
            return hex(lerp(0x1a, 0xff), lerp(0x0a, 0x6a), lerp(0x0a, 0x00));
        case 'cool':
            return hex(lerp(0x0a, 0x00), lerp(0x0a, 0x6a), lerp(0x1a, 0xff));
        case 'mono':
    ...
    try {
        ...
        for (const repo of repos) {
            for (const edge of repo?.languages?.edges ?? []) {
                ...
                if (cur) {
                    cur.size += size;
                    if (cur.color === '#8b949e' && color !== '#8b949e')
                        cur.color = color;
                }
                else {
