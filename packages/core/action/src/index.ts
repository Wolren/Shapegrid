...
function loadConfig(path: string): ShapegridConfig {
  const raw = readFileSync(resolve(path), 'utf8');
  return yaml.load(raw) as ShapegridConfig;
}
...
async function runGenerate(cfg: ShapegridConfig) {
  ...
  const boundary = loadBoundary(cfg.boundary);
