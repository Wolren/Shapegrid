...
function renderStats(container: HTMLElement, _id: string): void {
  ...
  const makeBig = (val: string, label: string) => {
    ...
    g.innerHTML = `<div style="font-size:18px;font-weight:700;color:var(--accent);line-height:1.2">${val}</div><div style="font-size:8px;color:var(--muted);text-transform:uppercase;letter-spacing:0.05em">${label}</div>`;
    return g;
  };
  ...
  detailRow.style.cssText = 'display:flex;gap:12px;font-size:9px;color:var(--muted);margin-bottom:6px';
  ...
  distLabels.style.cssText = 'display:flex;justify-content:space-between;font-size:8px;color:var(--muted);margin-top:2px';
